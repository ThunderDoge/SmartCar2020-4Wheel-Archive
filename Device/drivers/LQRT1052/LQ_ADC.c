...
{    
    ...
    */
    ...
    adcConfigStrcut.referenceVoltageSource = kADC_ReferenceVoltageSourceAlt0;
...
uint16_t ADC_Read_Average(ADCn_Ch_e CH,uint8_t times)
{
  uint32_t temp_sum = 0;
  uint32_t tmp_max = 0;
  uint32_t tmp_min = 4095;
  uint32_t tmp;
  uint8_t t;
  for(t=0;t<times;t++)
  {
    tmp = ADC_Read(ADC1, CH);
    if(tmp > tmp_max)
      tmp_max = tmp;
    if(tmp < tmp_min)
      tmp_min = tmp;
  ...
  return (temp_sum - tmp_max - tmp_min)/(times-2);
}
...
//        }
        //UART_PutStr(LPUART8,"\r\n-----------------------------\n");
            ...
            //volatile float lt = leftAdcVal / 940;
            sprintf(txt,"leftAdcVal:%d ",leftAdcVal);
            //UART_PutStr(LPUART8, txt);
            LCD_P6x8Str(4,1,txt);
            
            //volatile float rt = rightAdcVal / 1040;
            ...
            //volatile float mt = MidAdcVal / 690;
            sprintf(txt,"MidAdcVal:%d ",LMidAdcVal);
            //UART_PutStr(LPUART8, txt);
            LCD_P6x8Str(4,5,txt);
            
            sprintf(txt,"AdcError:%4.3f ",getAdcError());
            LCD_P6x8Str(4,6,txt);
            
            sprintf(txt,"MAdcError:%4.3f ",getMAdcError());
            LCD_P6x8Str(4,7,txt);
            
//            adcProcessAndGetValue(&queue);
...
//        if(((MidleftAdcVal /Ui_Max_MLeft_ADC_VAL - MidrightAdcVal / Ui_Max_MRight_ADC_VAL) > Ui_Turn_Adc) ||((MidleftAdcVal /Ui_Max_MLeft_ADC_VAL - MidrightAdcVal / Ui_Max_MRight_ADC_VAL) <  -Ui_Turn_Adc)){
//          if((MidleftAdcVal / Ui_Max_MLeft_ADC_VAL) > (MidrightAdcVal / Ui_Max_MRight_ADC_VAL))
//            purpostAdcError = purpostAdcError;
//          else
//            purpostAdcError = - purpostAdcError;
//          MAdcProcessAndGetValue();
//          LQ_SetServoDty(MAdc_Process_Result + SERVO_MID_PWM_VAL);
//        }
        //else{  
