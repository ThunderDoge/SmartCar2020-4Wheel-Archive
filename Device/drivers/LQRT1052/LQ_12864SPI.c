...
{ 
    ...
    while(Del_1ms--)
    {	
        for(i=0;i<3;i++) __asm("NOP");//30000是1ms,300是100K
    }
}
...
{      
    ...
    {  
        ...
        LCD_Rectangle(0, 0, 126,62,0);   //绘制矩形
        ...
        LCD_PrintU16(0, 2, 123);         //显示整数
        LCD_PrintFloat(0,3,-1234.567);    //显示浮点数
...
unsigned char oledBuf[8][100];
char * getOledBuf(unsigned char y) {
	//越界保护
	if (y >= 8) return (char *)oledBuf[7];
	else return (char *)oledBuf[y];
}
void LCD_refreshOneLine(unsigned char y, bool showReversely) {
	//越界保护
	if (y >= 8) return;
	
	unsigned char c = 0, i = 0;
	//写入前20个字符，遇到终止符前写入原字符,遇到终止符后写空格
...
void LCD_refreshHalfLine(unsigned char y, bool showReversely) {
	//越界保护
	if (y >= 8) return;
	
	unsigned char c = 0, i = 0;
	//写入前14个字符((128 - 40) / 6)，遇到终止符前写入原字符,遇到终止符后写空格
	bool writeSpace = false;
	for (unsigned char j = 0; j < 14; j++) {
		...
		//设置即将写入的位置(列,页), 这里留出前40列
		LCD_Set_Pos(j * 6 + 40, y);
		for (i = 0; i < 6; i++) {
			if (showReversely)
				LCD_WrDat(~F6x8[c][i]);
			else
