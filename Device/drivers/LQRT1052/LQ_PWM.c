...
void ServoInit(void)
{
    LQ_PWM_Init(PWM2, kPWM_Module_0, kPWM_PwmB,  75);//PWM的最低频率 = 6250 000 / VAL1  = 96Hz  A9
    LQ_SetServoDty(SERVO_MID_PWM_VAL);//LPF的莫名其妙要求，然后又删掉
}
...
void MotoInit(void)
{    
    LQ_PWM_Init(PWM1, kPWM_Module_3, kPWM_PwmA_B, 12000);//PWM的最低频率 = 6250 000 / VAL1  = 96Hz     L5 M5 电机1初始化
    LQ_PWM_Init(PWM2, kPWM_Module_3, kPWM_PwmA_B, 12000);//PWM的最低频率 = 6250 000 / VAL1  = 96Hz     M3 M4 电机2初始化
          ...
          case 0:        
            {
                ...
                if(pwm_channels == kPWM_PwmB || pwm_channels == kPWM_PwmA_B)  
                {
                    if(PWM2_MODULE0_PWMB == A9)
                    {
                        ...
                        IOMUXC_SetPinConfig(IOMUXC_GPIO_B0_07_FLEXPWM2_PWMB00, 0x10B0u);//原A9 因硬件问题修改为B9的初始化引脚
