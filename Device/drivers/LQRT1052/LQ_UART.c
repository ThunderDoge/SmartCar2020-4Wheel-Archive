...
void Test_UART(void)
{
    ...
    LCD_CLS();
    LED_Init();   
    LQ_UART_Init(LPUART6, 115200);
    LPUART_EnableInterrupts(LPUART6, kLPUART_RxDataRegFullInterruptEnable);
    LPUART_DisableInterrupts(LPUART6, kLPUART_IdleLineInterruptEnable);
    NVIC_SetPriority(LPUART6_IRQn,NVIC_EncodePriority(NVIC_GetPriorityGrouping(),1,2));
    EnableIRQ(LPUART6_IRQn);	                            //使能LPUART1中断LQ_UART_Init(LPUART6, 115200);   
...
//	uint8_t buff[10] = {'a','b','c','d','E','F','G'};
    while(1)
    {
      ...
      sprintf(txt,"tfMiniDistance:%d ",tfMiniDistance);
      LCD_P6x8Str(4,0,txt);
      
      sprintf(txt,"tfMiniStrength:%d ",tfMiniStrength);
      LCD_P6x8Str(4,1,txt);
//        
